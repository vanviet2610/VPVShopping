        ...
        foreach ($this->dataModel as $value) {
            if ($value['parent_id'] == $id) {
                if (!empty($subID) && $subID == $value['id']) {
                    $this->htmlOptions .= '<option selected value="' . $value['id'] . '">' . $subtext . ucwords($value['name'])  . '</option>';
                } else {
                    $this->htmlOptions .= '<option value="' . $value['id'] . '" >' . $subtext .  ucwords($value['name']) . '</option>';
